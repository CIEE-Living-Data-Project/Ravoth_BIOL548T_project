...
######################### MAKE THE FIGURE ######################### 
sal.fig <- sal.t %>%
  filter(count!=is.na(count), `Soil Temperature (°C)`!="N/A") %>%   # filtered out NAs
  ggplot(aes(x=as.numeric(`Soil Temperature (°C)`), y=as.numeric(count), col=Morph)) +  # changed the class for soil temperature & abundance count from character to numeric (or else doesn't plot properly)--this was plotted on the x axis, abundance plotted on y axis, and points coloured by redback salamander morph
